...
namespace QuantLib {
    ...
    std::complex<Real> FFTVarianceGammaEngine::complexFourierTransform(std::complex<Real> u) const
    {
        ...
        Real omega = log(1.0 - theta_ * nu_ - sigma_*sigma_ * nu_ / 2.0) / nu_;
        std::complex<Real> phi = exp(i1 * u * (log(s) + omega * t_)) 
            * pow(std::complex<Real>(dividendDiscount_/ riskFreeDiscount_,0.0), i1 * u);
        phi = phi * (pow((Real(1.0) - i1 * theta_ * nu_ * u + sigma_ * sigma_ * nu_ * u * u / Real(2.0)),
                         std::complex<Real>(-t_ / nu_, 0.0)));
