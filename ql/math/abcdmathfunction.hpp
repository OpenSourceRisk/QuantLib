...
namespace QuantLib {
    ...
    inline Real AbcdMathFunction::operator()(Time t) const {
        ...
        return t<0 ? 0.0 : (a_ + b_*t)*exp(-c_*t) + d_;
    }

    inline Real AbcdMathFunction::derivative(Time t) const {
        //return (da_ + db_*t)*exp(-c_*t);
        return t<0 ? 0.0 : (da_ + db_*t)*exp(-c_*t);
    }

    inline Real AbcdMathFunction::primitive(Time t) const {
        //return (pa_ + pb_*t)*exp(-c_*t) + d_*t + K_;
        return t<0 ? 0.0 : (pa_ + pb_*t)*exp(-c_*t) + d_*t + K_;
    }
