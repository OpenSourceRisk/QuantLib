    ...
    Real GammaDistribution::operator()(Real x) const {
        ...
        if (x<(a_+1.0)) {
            ...
            for (Size n=1; n<=100; n++) {
                ...
                del *= x/ap;
                sum += del;
                if (abs(del) < abs(sum)*3.0e-7)
                    return sum*exp(-x + a_*log(x) - gln);
            }
        } else {
            Real b = x + 1.0 - a_;
            Real c = QL_MAX_REAL;
            ...
            for (Size n=1; n<=100; n++) {
                ...
                Real del = d*c;
                h *= del;
                if (abs(del - 1.0)<QL_EPSILON)
                    return 1.0-h*exp(-x + a_*log(x) - gln);
            }
        }
    ...
    Real GammaFunction::value(Real x) const {
        if (x >= 1.0) {
            ...
            if (x > -20.0) {
                ...
                return -M_PI/(value(-x)*x*sin(M_PI*x));
            }
        }
    }
}
