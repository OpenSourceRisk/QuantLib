...
namespace QuantLib {
    ...
    Real BivariateCumulativeNormalDistributionDr78::operator()(Real a,
        ...
        Real MaxCumNormDistAB = max(CumNormDistA, CumNormDistB);
        Real MinCumNormDistAB = min(CumNormDistA, CumNormDistB);
        ...
        Real a1 = a / sqrt(2.0 * (1.0 - rho2_));
        Real b1 = b / sqrt(2.0 * (1.0 - rho2_));
        ...
        if (a<=0.0 && b<=0 && rho_<=0) {
            ...
            for (Size i=0; i<5; i++) {
                for (Size j=0;j<5; j++) {
                    sum += x_[i]*x_[j]*
                        exp(a1*(2.0*y_[i]-a1)+b1*(2.0*y_[j]-b1)
                                 +2.0*rho_*(y_[i]-a1)*(y_[j]-b1));
                }
            }
            result = sqrt(1.0 - rho2_)/M_PI*sum;
        ...
        class eqn3 { /* Relates to eqn3 Genz 2004 */
          public:
            ...
            Real operator()(Real x) const {
                Real sn = sin(asr_ * (-x + 1) * 0.5);
                return exp((sn * hk_ - hs_) / (1.0 - sn * sn));
            }
          private:
            Real hk_, asr_, hs_;
        };
    ...
    Real BivariateCumulativeNormalDistributionWe04DP::operator()(
        ...
        if (abs(correlation_) < 0.3) {
            gaussLegendreQuad.order(6);
        } else if (abs(correlation_) < 0.75) {
        ...
        if (abs(correlation_) < 0.925)
        {
            if (abs(correlation_) > 0)
            {
                Real asr = asin(correlation_);
                eqn3 f(h,k,asr);
